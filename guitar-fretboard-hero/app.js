(() => {
  ...
  const state={screen:'home',mode:'penta',root:'A',quality:'minor',pattern:'all',shapeIndex:0,quiz:null,quizReveal:null};
  ...
  function go(screen){state.screen=screen;$$('.screen').forEach(x=>x.classList.remove('active'));$('#'+screen).classList.add('active');if(screen==='practice') renderPractice();if(screen==='quiz') startQuiz();}
  ...
  window.addEventListener('resize',()=>{if(state.screen==='practice')renderPractice();if(state.screen==='quiz')renderQuizBoard()});
  ...
  function degreeLabel(pc){const d=degreeFor(pc);return d==='root'?'R':d==='third'?(state.quality==='minor'?'♭3':'3'):d==='fifth'?'5':d==='seventh'?(mod(pc-rootPC())===10?'♭7':'7'):d==='ninth'?'9':'•'}
  ...
  function patternWindows(){
    // Standard connected minor-pentatonic box layout. Major uses the relative-minor geometry.
    const anchorQuality=state.quality==='minor'?rootPC():mod(rootPC()-3);
    const lowE=7; let r=mod(anchorQuality-lowE); // root fret on low E within first octave
    const offsets=[[0,3],[3,5],[5,8],[8,10],[10,12]];
    const out=[];
    for(const [idx,o] of offsets.entries()){
      for(const shift of [-12,0,12,24]){const a=r+o[0]+shift,b=r+o[1]+shift;if(b>=0&&a<=24)out.push({id:idx+1,a:Math.max(0,a),b:Math.min(24,b)})}
    }
    return out;
  }
  ...
  function triadShapes(){
    ...
    for(let s=0;s<=3;s++){
      const candidates=[];
      for(let i=0;i<3;i++){const arr=[];for(let f=0;f<=24;f++){const pc=noteAt(s+i,f);const di=pcs.indexOf(pc);if(di>=0)arr.push({string:s+i,fret:f,pc,degree:labels[di],midi:tuning[s+i].midi+f})}candidates.push(arr)}
  ...
  function chordShapes(){
    ...
    const pcs=[rootPC(),thirdPC(),fifthPC()]; const labels=['root','third','fifth']; const shapes=[];
    for(let s=0;s<=2;s++){
      const cand=[];for(let i=0;i<4;i++){const arr=[];for(let f=0;f<=24;f++){const pc=noteAt(s+i,f),di=pcs.indexOf(pc);if(di>=0)arr.push({string:s+i,fret:f,pc,degree:labels[di],midi:tuning[s+i].midi+f})}cand.push(arr)}
  ...
  function getShapes(){return state.mode==='triad'?triadShapes():state.mode==='chord'?chordShapes():[]}
  ...
  function renderFretboard(svg,opt){
    const isP=portrait(), W=isP?520:1500,H=isP?1500:430;svg.setAttribute('viewBox',`0 0 ${W} ${H}`);svg.innerHTML='';
    ...
    const fretPos=f=>fretStart+(fretEnd-fretStart)*(f/24), stringPos=s=>stringStart+(stringEnd-stringStart)*(s/5), visualStringPos=s=>stringPos(isP?s:5-s);
    ...
    if(opt.mode==='penta'){
      ...
      patternWindows().forEach(w=>{if(state.pattern!=='all'&&String(w.id)!==String(state.pattern))return;const a=fretPos(w.a),b=fretPos(w.b);const attrs=isP?{x:45,y:a,width:W-90,height:Math.max(8,b-a),fill:colors[w.id-1]+'15',stroke:colors[w.id-1], 'stroke-width':2,rx:12}:{x:a,y:32,width:Math.max(8,b-a),height:H-62,fill:colors[w.id-1]+'15',stroke:colors[w.id-1],'stroke-width':2,rx:12};svg.append(svgEl('rect',attrs));const tx=isP?62:a+8,ty=isP?a+22:52;svg.append(svgEl('text',{x:tx,y:ty,fill:colors[w.id-1],'font-size':14,'font-weight':900},`P${w.id}`))})
    }
    ...
    for(let f=0;f<=24;f++){
      ...
      svg.append(line);
      if([0,3,5,7,9,12,15,17,19,21,24].includes(f)){
    ...
    // inlays
    [3,5,7,9,12,15,17,19,21].forEach(f=>{const p=(fretPos(f-1)+fretPos(f))/2;if(f===12){[-14,14].forEach(o=>svg.append(svgEl('circle',isP?{cx:W/2+o,cy:p,r:5,fill:'#59636b'}:{cx:p,cy:H/2+o,r:5,fill:'#59636b'})))}else svg.append(svgEl('circle',isP?{cx:W/2,cy:p,r:5,fill:'#59636b'}:{cx:p,cy:H/2,r:5,fill:'#59636b'}))});
    ...
    for(let s=0;s<6;s++)for(let f=0;f<=24;f++){
      ...
      const d=degreeFor(pc);let col=d==='root'?'#27d7ff':d==='third'?'#ff3ec9':d==='fifth'?'#ffd53c':d==='seventh'?'#62ef75':d==='ninth'?'#ff8b3d':'#99a7b2';let opacity=d==='other'?.42:1;
  ...
  function updateQuizStats(){if(!state.quiz)return;$('#attemptCount').textContent=state.quiz.attempts;$('#correctCount').textContent=state.quiz.correct;$('#quizAttempts').textContent=`${state.quiz.attempts} ATTEMPTS`}
  ...
  function renderFretboardQuiz(svg,q){
    const isP=portrait(),W=isP?520:1500,H=isP?1500:430;svg.setAttribute('viewBox',`0 0 ${W} ${H}`);svg.innerHTML='';const fretStart=isP?110:90,fretEnd=isP?H-70:W-40,stringStart=isP?85:70,stringEnd=isP?W-55:H-48;const fretPos=f=>fretStart+(fretEnd-fretStart)*(f/24),stringPos=s=>stringStart+(stringEnd-stringStart)*(s/5),visualStringPos=s=>stringPos(isP?s:5-s);
